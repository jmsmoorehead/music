...
```clj
> (use 'overtone.live)
> (boot) ; boot the SuperCollider synthesis engine
> (hit)  ; play a simple synth
```

If you hear a ping sound when you run @(hit)@ than you have a working setup.
...
The synth macro takes a synthesizer definition form.  It will compile this form
...
Synths can take arguments: 

```clj
> (synth [freq 220] (saw freq))
> (*1 110)
5
> (kill 5)
```

and you can use @defsynth@ to create named synths:

```clj
> (defsynth foo [freq 440] (* 0.3 (saw freq)))
> (foo 220)
> (foo 660)
> (reset) ; stop all running synths
```

Remember @(reset)@, as it can save you when accidentally trigger a bunch of
